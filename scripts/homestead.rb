...
class Homestead
  def self.configure(config, settings)
    ...
    config.vm.provision 'shell' do |s|
      ...
      s.inline = 'sudo chown -R vagrant:vagrant /usr/local/bin && sudo -u vagrant /usr/bin/php8.5 /usr/local/bin/composer self-update --no-progress && sudo chown -R vagrant:vagrant /home/vagrant/.config/'
      s.privileged = false
    end
